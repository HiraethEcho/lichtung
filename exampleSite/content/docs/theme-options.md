...
控制首页、章节页、标签页的文章列表。列表由两个独立选项控制：`ListStyle`（如何组织）与 `ItemStyle`（每项如何渲染）。

| 参数             | 说明           | 可选值                        |
| ---------------- | -------------- | ----------------------------- |
| `ListStyle`      | 列表组织方式   | `'list'`, `'date'`, `'split'` |
| `ItemStyle`      | 每项渲染样式   | `'card'`, `'simple'`          |
...
**ItemStyle**（对所有 ListStyle 生效）：

| 值       | 效果                                       |
| -------- | ------------------------------------------ |
| `card`   | 文章卡片，显示标题、日期、标签等 meta 信息 |
| `simple` | 极简条目——只有标题链接、日期、摘要         |
...
| `ListStyle`        | string | 列表组织方式（覆盖全局）                          |
| `ItemStyle`        | string | 每项渲染样式（覆盖全局）                          |
